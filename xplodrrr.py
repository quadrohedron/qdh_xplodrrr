...
#			self.timestamp=TS
			...
			for j in range(rlen+len(self.ghostps)):
				...
				if self.boundson:
					...
					i.p.y%=704
				if j>=rlen:					
					if not sum(map(lambda x:abs(x.p-i.p)<10, self.ghostps[:j-rlen]+self.ghostps[j+1-rlen:] +self.realps)):
						reloc.append(j-rlen)
				if self.reson:
		...
		for i in self.ghostps+self.realps:
			scdr.fill(i.c)
			scdr.ellipse(i.p.x-8,i.p.y-8,16,16)
		if self.mainview:
